...
sudo docker run --detach --env-file .env \
  --network silver-potato --hostname postgis --name postgis --publish 5439:5432 \
  postgis:latest
sudo docker run --detach --env-file .env \
  --network silver-potato --hostname rstats --name rstats --publish 8787:8787 \
  rstats:latest
sudo docker run --detach --env-file .env \
  --network silver-potato --hostname pgadmin4 --name pgadmin4 --publish 8686:8686 \
